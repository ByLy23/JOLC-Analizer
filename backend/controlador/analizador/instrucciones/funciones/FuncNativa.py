    ...
    def interpretar(self, arbol, tablaSimbolo):
        ...
        if self.argumentos["exp2"] != None:
            ...
            if exp == "parse":
                ...
                if self.argumentos["exp2"].tipo == TipoDato.DECIMAL:
                    if arg1 == "Int64":
                        self.tipo = TipoDato.ENTERO
                        return trunc(arg2)
                    else:
                ...
                if self.argumentos["exp1"].tipo == TipoDato.ARREGLO and self.argumentos["exp2"].tipo == TipoDato.ARREGLO:
                    key = int(list(arg1)[-1])+1
                    print(self.argumentos["exp2"].expresiones[0].valor)
                    expresion = self.argumentos["exp2"].expresiones[0]
                    simbolo = Simbolo(
                        str(key), expresion.tipo, expresion.valor)
